...
isset($_POST['hodnota']) ? $hodnotaZasilky = $_POST['hodnota'] : $hodnotaZasilky = 1;
       ...
       echo '<div id="naPoste">
            ...
            <a href="https://www.ceskaposta.cz/sluzby/baliky/cr/balik-do-balikovny" target="_blank"><img src="/img/CP-logo-Balikovna-1.jpg" alt="Balík do balíkovny" class="fr" /></a>
            <div class="cl"></div>';
        ...
        if (count($data) == 0) {
            ...
            foreach ($data as $x) {
                if ($x['V_PROVOZU'] == 0) {
					...
					<input type="hidden" name="_posta2" value="'.$x['ID'].'" />
					<input type="hidden" name="_posta2_psc" value="'.$x['PSC'].'" />
					<input type="hidden" name="_posta2_nazev" value="'.$x['NAZ_PROV'].'" />
					<input type="hidden" name="_posta2_adresa" value="'.$x['ADRESA'].'" />
