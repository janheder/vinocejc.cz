...
class QRPlatba  
{
	...
	public function getQRCodeImage($t)
    {
        $chunks = array('SPD', self::VERSION);
		...
		$qr_text =  implode('*', $chunks);

        include_once './tridy/phpqrcode/qrlib.php'; 
        $path = './fotky/qr/'; 
		$file = $path.uniqid().".png"; 
		$ecc = 'M'; 
		$pixel_Size = 8; 
		$frame_Size = 5; 
		QRcode::png($qr_text, $file, $ecc, $pixel_Size, $frame_Size);
		if($t==2)
		{
			 // absolutní URL pro emailovou rekapitulaci
			 return '<img src="'.__URL__.substr($file, 1).'" alt="QR platba">';
		}
		else
		{
			 return '<img src="'.$file.'" alt="QR platba">';	
		}
